...
class LeadScoringApplicator:
    ...
    def calculate_lead_score(self, lead):
        ...
        completeness = 0
        if lead.get('name', '').strip(): completeness += 20
        if lead.get('email', '') and '@' in lead.get('email', ''): completeness += 25
        if lead.get('phone', '').strip(): completeness += 20
        if lead.get('website', '') and ('http' in lead.get('website', '') or '.' in lead.get('website', '')): completeness += 20
        if lead.get('location', '').strip(): completeness += 10
        if lead.get('business_type', '').strip(): completeness += 5
        completeness = min(completeness, 100)
        
        # Verifizierungs-Score
        verification = 0
        if lead.get('verified', False): verification += 80
        if lead.get('ai_score', 0) > 0: verification += min(lead.get('ai_score', 0) / 10, 20)
        if lead.get('quality_score', 0) > 0: verification = max(verification, lead.get('quality_score', 0))
        ...
        if lead.get('revenue_estimate', 0) > 0:
            ...
            if revenue > 1000000: business_value += 20
            elif revenue > 500000: business_value += 15
            elif revenue > 100000: business_value += 10
            else: business_value += 5
        ...
        if 'working_lead_scraper' in source: source_score += 30
        elif 'crawl4ai' in source: source_score += 25
        elif 'playwright' in source: source_score += 20
        elif 'google_maps' in source: source_score += 15
        elif 'manual' in source or 'verified' in source: source_score += 40
        
        if lead.get('auto_integrated', False): source_score += 10
        ...
        if lead.get('created_at'):
            try:
                ...
                if days_diff <= 1: freshness_score = 100
                elif days_diff <= 7: freshness_score = 90
                elif days_diff <= 30: freshness_score = 70
                elif days_diff <= 90: freshness_score = 50
                elif days_diff <= 180: freshness_score = 30
...
## 🎯 Score Distribution
- **Hot Leads (80+):** {len([l for l in leads if l.get('calculated_score', 0) >= 80])}
- **Warm Leads (60-79):** {len([l for l in leads if 60 <= l.get('calculated_score', 0) < 80])}
- **Cold Leads (40-59):** {len([l for l in leads if 40 <= l.get('calculated_score', 0) < 60])}
- **Poor Leads (<40):** {len([l for l in leads if l.get('calculated_score', 0) < 40])}
